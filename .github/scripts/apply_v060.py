...
if 'private fun PaymentTypePicker' in text and 'var showClearDialog by remember' in text and 'private fun ReimbursementDialog' in text:
    print('v0.6 UI already applied; nothing to patch')
    raise SystemExit(0)

def rep(old, new, count=1):
    global text
    if old not in text:
        raise SystemExit(f'Patch target not found: {old[:120]!r}')
    text = text.replace(old, new, count)

rep('import androidx.compose.material3.CardDefaults\n', 'import androidx.compose.material3.CardDefaults\nimport androidx.compose.material3.Checkbox\n')
rep('import za.co.eci.slipmanager.data.MoneyReturn\n', 'import za.co.eci.slipmanager.data.MoneyReturn\nimport za.co.eci.slipmanager.data.PaymentType\nimport za.co.eci.slipmanager.data.Reimbursement\n')

rep('    val returns by repository.returns.collectAsState()\n', '    val returns by repository.returns.collectAsState()\n    val reimbursements by repository.reimbursements.collectAsState()\n')
rep('Screen.REPORTS -> ReportsScreen(repository, advances, slips, returns)', 'Screen.REPORTS -> ReportsScreen(repository, advances, slips, returns, reimbursements)')
rep('Screen.SETTINGS -> SettingsScreen(repository, advances, slips, returns)', 'Screen.SETTINGS -> SettingsScreen(repository, advances, slips, returns, reimbursements)')
...
rep('MiniTotal("Received", rec.receivedCents, Modifier.weight(1f))', 'MiniTotal("Received", activeRec.receivedCents, Modifier.weight(1f))')
rep('MiniTotal("Slips", rec.slipsCents, Modifier.weight(1f))', 'MiniTotal("Slips", activeRec.slipsCents, Modifier.weight(1f))')
rep('MiniTotal("Returned", rec.returnedCents, Modifier.weight(1f))', 'MiniTotal("Returned", activeRec.returnedCents, Modifier.weight(1f))')
...
new = '''        item {
        ...
        item {
            Button(onClick = onScan, modifier = Modifier.fillMaxWidth().height(56.dp)) {'''
if old not in text:
    raise SystemExit('Home own-money insertion point not found')
text = text.replace(old, new, 1)

marker = '\n@Composable\nprivate fun AdvanceSelectorRow'
if marker not in text:
    raise SystemExit('AdvanceSelectorRow marker not found')
replacement = '''
...
text = text.replace(
    'slips.filter { it.advanceId == advance.id }.sumOf { it.totalCents }',
    'slips.filter { it.advanceId == advance.id }.sumOf { it.companyPaidCents }'
)

return_marker = '@Composable\nprivate fun ReturnMoneyDialog'
reimbursement_dialog = '''@Composable
...
if return_marker not in text:
    raise SystemExit('ReturnMoneyDialog marker not found')
text = text.replace(return_marker, reimbursement_dialog + return_marker, 1)

edit_start = '@Composable\nprivate fun EditSlipScreen'
picker_start = '@Composable\nprivate fun AdvancePicker'
before, after = text.split(edit_start, 1)
edit, post = after.split(picker_start, 1)

old = '    var paymentRef by remember(original.id, original.imagePath) { mutableStateOf(original.paymentReference) }\n'
new = '''    var paymentRef by remember(original.id, original.imagePath) { mutableStateOf(original.paymentReference) }
...
'''
if old not in edit:
    raise SystemExit('Edit payment state target not found')
edit = edit.replace(old, new, 1)

old = '''        TextFieldSimple("Payment reference", paymentRef, { paymentRef = it })
        AdvancePicker(advances, selectedAdvanceId, { selectedAdvanceId = it })'''
new = '''        TextFieldSimple("Payment reference", paymentRef, { paymentRef = it })
...
if old not in edit:
    raise SystemExit('Edit payment controls target not found')
edit = edit.replace(old, new, 1)

old = '''                val totalCents = parseMoney(total)
                val epoch = parseDate(date)
                if (totalCents == null || totalCents <= 0) {
                    Toast.makeText(context, "Total is required for reconciliation.", Toast.LENGTH_SHORT).show()
                } else {'''
new = '''                val totalCents = parseMoney(total)
                val epoch = parseDate(date)
...
if old not in edit:
    raise SystemExit('Edit save validation target not found')
edit = edit.replace(old, new, 1)
edit = edit.replace('                            advanceId = selectedAdvanceId,', '                            advanceId = if (paymentType == PaymentType.OWN) null else selectedAdvanceId,', 1)
edit = edit.replace(
    '                            paymentReference = paymentRef.trim()\n',
    '                            paymentReference = paymentRef.trim(),\n                            paymentType = paymentType,\n                            ownMoneyCents = ownCents\n',
    1
)
text = before + edit_start + edit + picker_start + post

payment_picker = '''@Composable
...
'''
text = text.replace(picker_start, payment_picker + picker_start, 1)
...
rep('PdfExporter.officePack(context, label, selectedAdvances, selectedSlips, selectedReturns)', 'PdfExporter.officePack(context, label, selectedAdvances, selectedSlips, selectedReturns, selectedReimbursements)')
...
rep('val file = BackupManager.createBackup(context, advances, slips, returns)', 'val file = BackupManager.createBackup(context, advances, slips, returns, reimbursements)')
rep('repository.replaceAll(restored.advances, restored.slips, restored.returns)', 'repository.replaceAll(restored.advances, restored.slips, restored.returns, restored.reimbursements)')
...
new = '''        Spacer(Modifier.height(12.dp)); SectionTitle("Data management")
...
if old not in text:
    raise SystemExit('Settings data-management insertion target not found')
text = text.replace(old, new, 1)

settings_end_marker = '\n}\n\n@Composable\nprivate fun TextFieldSimple'
if settings_end_marker not in text:
    raise SystemExit('Settings end marker not found')
clear_dialog = '''
...
'''
text = text.replace(settings_end_marker, clear_dialog + settings_end_marker, 1)

path.write_text(text)
print('v0.6 UI patch applied')
